...
class PNN:
    def __init__(self, train_loader, test_loader, embed_size=10, product_way='in',
                 layer_size=None, 
        ...
        if self.product_way == 'out':
            self.var_list.append(['kernel', [self.embed_size, num_pairs, self.embed_size], 'xavier'])
...
#         with self.graph.as_default():
        ...
        with tf.variable_scope('Product_Layer'):
            ...
            if self.product_way == 'in':
                product = tf.reshape(tf.reduce_sum(p * q, [-1]), [-1, num_pairs])
            else:
